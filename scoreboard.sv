...
class scoreboard extends uvm_scoreboard;
    ...
    task validate_outputs(my_transaction inp, my_transaction out);
        ...
        if (inp.compare(out)) begin
            ...
            `uvm_info("SCOREBOARD",
                $sformatf("DUT: BRESP=%0h, RDATA=%0h, RRESP=%0h\nREF: BRESP=%0h, RDATA=%0h, RRESP=%0h\n\n",
                out.BRESP, out.RDATA, out.RRESP,
                inp.BRESP, inp.RDATA, inp.RRESP),
                UVM_NONE)
        end
        else begin
            ++MISMATCH;
    ...
    task wr_task(my_transaction tr);
        if ((!wr_add) && tr.AWREADY && tr.AWVALID) begin
            held.AWADDR = tr.AWADDR;
            held.AWPROT = tr.AWPROT;
            wr_add = 1;
        end

        if ((!wr_data) && tr.WREADY && tr.WVALID) begin
            held.WDATA = tr.WDATA;
            held.WSTRB = tr.WSTRB;
            wr_data = 1;
        end
    ...
    task wr_operation(my_transaction tr);
        if (held.AWADDR > 63) begin
        ...
        else if ((held.AWADDR > `AW'h24 && held.AWADDR < `AW'h34) ||
                 (held.AWADDR[1:0] != 00)) begin
            tr.BRESP = 2'b10;
        end
        else begin
            tr.BRESP = 00;
            ...
            if (held.AWPROT == 00) begin
                if (held.WSTRB[3])
                    mem[held.AWADDR + 3] = held.WDATA[31:24];

                if (held.WSTRB[2])
                    mem[held.AWADDR + 2] = held.WDATA[23:16];

                if (held.WSTRB[1])
                    mem[held.AWADDR + 1] = held.WDATA[15:8];

                if (held.WSTRB[0])
                    mem[held.AWADDR + 0] = held.WDATA[7:0];
            end
        end
    endtask
    ...
    task rd_operation(my_transaction tr);
        ...
        else if ((held.ARADDR > `AW'h30 && held.ARADDR < `AW'h3C) ||
                 (held.ARADDR[1:0] != 00)) begin
            tr.RRESP = 2'b10;
            tr.RDATA = {`DW{1'b0}};
        end
        else begin
