...
class sequences extends uvm_sequence #(my_transaction,my_transaction);
	...
	task gen_next();
		...
		aw_pend=rsp.AWVALID&&!rsp.AWREADY;
		w_pend=rsp.WVALID&&!rsp.WREADY;
		ar_pend=rsp.ARVALID&&!rsp.ARREADY;

		if(req.randomize() with {
			aw_pend -> (AWVALID==1 && AWADDR==rsp.AWADDR && AWPROT==rsp.AWPROT);
			w_pend -> (WVALID==1 && WDATA==rsp.WDATA && WSTRB==rsp.WSTRB);
			ar_pend -> (ARVALID==1 && ARADDR==rsp.ARADDR && ARPROT==rsp.ARPROT);
		})
			log_req();
		else
			`uvm_error("SEQ","SEQ failed");
...
class write_aw_then_w_seq extends sequences;
	...
	task force_aw_then_w();
		...
		if(rsp!=null) begin
			if(rsp.AWVALID&&!rsp.AWREADY)
				req.AWVALID=1'b1;
			if(rsp.WVALID&&!rsp.WREADY)
				req.WVALID=1'b1;
